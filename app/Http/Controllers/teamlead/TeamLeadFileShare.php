...
class TeamLeadFileShare extends Controller
{
    // single file share module
 ...
 ';
    ...
    public function shared()
    {
        ...
        $folders = DB::table('shares')
            ...
            ->distinct()
            ->get();
        return view('teamlead.pages.shared.shared', compact('folders'));
    }
    public function sharedFileList()
    {
        ...
        $files = DB::table('files')
            ...
            ->select(
                ...
                'shares.share_from',
                'shares.created_at as shared_time',
                'users.name as shared_from_user'
            )
            ->whereNull('shares.folder_id')
            ->paginate(10);

        return view('teamlead.pages.shared.shared-list', compact('files'));
    }
    ...
    public function sharedFolderFileList($folderId = null)
    {
        ...
        if ($folder) {
            ...
            // Get both trashed and non-trashed files
            $files = File::withTrashed()->where('folder_id', $folderId)->paginate(10);
        } else {
            $folderName = null;
            $files = collect();
        }
        return view('teamlead.pages.shared.shared-filelist-in-folder', compact('folderId', 'folderName', 'files'));
    }
}
