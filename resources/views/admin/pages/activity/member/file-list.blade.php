                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <h1 class="card-title pb-5 ">
                                <span class="text-danger"> Deleted Files - </span> Members
                            </h1>
                            <div class="pe-5">
                                @foreach ($deletedFiles as $file)
                                    <p> Name : <span class="text-success ps-2">{{ $file->user->name }}</span></p>
                                    <p>Department : <span
                                            class="text-success ps-2">{{ $file->user->department->dept_name }}</span></p>
                                    @php
                                        break;
                                    @endphp
                                @endforeach
                            ...
                            <tbody>
                                @forelse($deletedFiles as $index => $file)
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{ url('admin/activity/members/delete/' . $file->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                ...
                                                <button type="submit"
                                                    ...
                                                    onclick="return confirm('sure want to remove file from site ?')">Delete</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
